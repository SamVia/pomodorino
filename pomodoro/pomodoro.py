...
#sounds
work_sound = open(r"pomodoro\\work.wav").read()
...
if not st.session_state.submitted:
    page_bg_img = f""" <style> [data-testid="stAppViewContainer"] > .main {{ background-image: url({link_start}); background-size: 100% auto; background-position: center center; background-repeat: no-repeat; background-attachment: local; margin-top:-0; }} [data-testid="stHeader"] {{ background: rgba(0,0,0,0); }} </style> """
    ...
    for i in range(0,st.session_state.inputs[2]):
        ...
        for s in range(seconds, 0, -1):
            print_m = f"""<div style="display: flex; justify-content: center; align-items: center; height: 100vh; font-size: 80px; font-weight: bold;"> {timedelta(seconds=s)}</div>"""
            print_container.markdown(print_m, unsafe_allow_html=True)
            time.sleep(1)
        
        print_container.empty()
         
        page_bg_img = f"""<style>[data-testid="stAppViewContainer"] > .main {{background-image: url({link_pause});background-size: 100% auto;background-position: center center;background-repeat: no-repeat;background-attachment: local;margin-top:{str(-165+i*pixel_adj+6)}px;}}[data-testid="stHeader"] {{background: rgba(0,0,0,0);}}</style>"""
        st.markdown(page_bg_img, unsafe_allow_html=True)
